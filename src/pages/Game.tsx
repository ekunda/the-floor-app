...
import { useGameStore, computeStats } from '../store/useGameStore'
  ...
  if (appState === 'restoring') {
    return (
      <div style={{ ...styles.fullscreen, flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{
          fontSize: '0.75rem', letterSpacing: 4,
...
function OpBtn({ children, onClick, gold }: {
...
const styles: Record<string, React.CSSProperties> = {
  ...
  splashContent: {
    ...
    alignItems: 'center', gap: 14, textAlign: 'center', padding: '0 24px',
  },
  splashLogo: {
  ...
  splashBtn: {
    ...
    border: '2px solid #D4AF37', color: '#D4AF37', borderRadius: 60,
    cursor: 'pointer', transition: 'all .25s ease', animation: 'btnPulse 2s ease-in-out infinite',
  },
  ...
  headerLogo: {
    fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(2rem, 5vw, 3.2rem)',
    letterSpacing: 16, background: 'linear-gradient(135deg, #FFD700, #C0C0C0)',
    WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent', lineHeight: 1,
  },
  headerSub: { fontSize: '.68rem', letterSpacing: 8, color: '#4a4a4a', textTransform: 'uppercase', marginTop: 2 },
  boardArea: {
    flex: 1, width: '100%', minHeight: 0, padding: '8px 16px',
  ...
  toast: {
    ...
    transform: 'translate(-50%, -50%)', background: '#FFD700', color: '#000',
    fontFamily: "'Bebas Neue', sans-serif", fontSize: '1.8rem', letterSpacing: 6,
    padding: '12px 36px', borderRadius: 50,
    boxShadow: '0 0 60px rgba(212,175,55,.5)', zIndex: 9999, pointerEvents: 'none',
  },
}
